...
func (s *Store) DeleteDomain(ctx context.Context, msg *types.MsgDeleteDomain) error {
	sqlStatement := `
		UPDATE domains SET deleted_at = now() 
		WHERE name = $1`
	_, err := s.db.ExecContext(ctx, sqlStatement, msg.Domain)
	return err
}

func (s *Store) TransferDomain(ctx context.Context, msg *types.MsgTransferDomain) error {
	sqlStatement := `
	UPDATE domains
	SET admin = $1
	WHERE name = $2`
	_, err := s.db.ExecContext(ctx, sqlStatement, msg.NewAdmin, msg.Domain)
	return err
}

func (s *Store) RegisterAccount(ctx context.Context, msg *types.MsgRegisterAccount) (int64, error) {
	var accountID int64

	sqlStatement := `SELECT id FROM domains WHERE name = $1;`
	row := s.db.QueryRowContext(ctx, sqlStatement, msg.Domain)
	var domainID int64
...
func (s *Store) ReplaceAccountResources(ctx context.Context, msg *types.MsgReplaceAccountResources) (int64, error) {
	var resourceID int64

	sqlStatement := `SELECT id FROM accounts WHERE domain = $1 and name = $2;`
	row := s.db.QueryRowContext(ctx, sqlStatement, msg.Domain, msg.Name)
	var accountID int64
...
func (s *Store) DeleteAccount(ctx context.Context, msg *types.MsgDeleteAccount) error {
	sqlStatement := `
		UPDATE accounts SET deleted_at = now() 
		WHERE domain = $1 AND name = $2`
	_, err := s.db.ExecContext(ctx, sqlStatement, msg.Domain, msg.Name)
	return err
}
