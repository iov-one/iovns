...
func GetQueryCmd(queryRoute string, cdc *codec.Codec) *cobra.Command {
	...
	configQueryCmd := &cobra.Command{
		...
		Short:                      fmt.Sprintf("Querying commands for the %s module", configuration.ModuleName),
...
func getCmdQueryConfig(route string, cdc *codec.Codec) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			path := fmt.Sprintf("custom/%s/%s", route, configuration.QuerierRoute)
			resp, _, err := cliCtx.Query(path)
			if err != nil {
				return err
			}
			var jsonResp configuration.Config // TODO make this a separate type
