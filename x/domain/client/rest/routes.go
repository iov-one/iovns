...
func registerQueryRoutes(cliCtx context.CLIContext, r *mux.Router, queryPath, storeName string, queries []iovns.QueryHandler) {
	for _, query := range queries {
		path := fmt.Sprintf("/%s/query/%s", queryPath, query.QueryPath())
...
func RegisterRoutes(cliContext context.CLIContext, r *mux.Router, queryPath, storeName string, queries []iovns.QueryHandler) {
	// register tx routes
	registerTxRoutes(cliContext, r, storeName)
	// register query routes
	registerQueryRoutes(cliContext, r, queryPath, storeName, queries)
}
