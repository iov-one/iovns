...
func handleMsgRegisterDomain(ctx sdk.Context, keeper Keeper, msg MsgRegisterDomain) (resp *sdk.Result, err error) {
	// check if domain exists
	if _, ok := keeper.GetDomain(ctx, msg.Name); ok {
		err = sdkerrors.Wrap(ErrDomainAlreadyExists, msg.Name)
		return
	}
	...
	if !regexp.MustCompile(keeper.ConfigurationKeeper.GetValidDomainRegexp(ctx)).MatchString(msg.Name) {
		err = sdkerrors.Wrap(ErrInvalidDomainName, msg.Name)
		return
	}
	...
	if !msg.HasSuperuser && !msg.Admin.Equals(keeper.ConfigurationKeeper.GetOwner(ctx)) {
		err = sdkerrors.Wrapf(ErrUnauthorized, "%s is not allowed to register a domain without a superuser", msg.Admin)
		return
	}
	// set new domain
	domain := Domain{
